...
@section('content')
    @component('backend._components.form_container', ["action" => $form_action, "page_type" => $page_type, "files" => false])
        ...
        @if(!Auth::guard('user')->check())
            @component('backend._components.input_radio', ['name' => 'status', 'options' => [__('label.pending'), __('label.active'), __('label.block')], 'label' => __('label.status'), 'required' => 1, 'value' => $item->status, 'isDisabled' => $isApproval]) @endcomponent
        @endif
        @if($isApproval)

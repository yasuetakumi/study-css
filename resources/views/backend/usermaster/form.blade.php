...
@section('breadcrumbs')
    <ol class="breadcrumb float-sm-right">
        @if($isAdminLogin)
            ...
            <li class="breadcrumb-item"><a href="{{route('admin.user.index')}}"></i>@lang('label.user') @lang('label.list')</a></li>
        @else
...
@section('content')
    @component('backend._components.form_container', ["action" => $form_action, "page_type" => $page_type, "files" => false])
        ...
        @component('backend._components.input_select', ['name' => 'belong_company_id', 'options' => $companies, 'label' => __('label.company'), 'required' => 1, 'value' => $item->belong_company_id]) @endcomponent

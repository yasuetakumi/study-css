<div id="form-group--{{ $name }}" class="row form-group">
    ...
    <div class="col-xs-12 col-sm-12 col-md-9 col-lg-10 col-content">
        ...
        <input step="0.0001" placeholder="{{ isset($placeholder) ? $placeholder : '' }}" v-on:change="{{isset($method) ? $method : ''}}" type="number" id="input-{{ $name }}" name="{{ $name }}" class="form-control @error($name) is-invalid @enderror" value="{{ !empty($value) ? $value : old($name) }}" {{ !empty($required) ? 'required' : '' }} {{isset($isReadOnly) && $isReadOnly == true ? 'readonly' : '' }} />
    </div>
</div>
